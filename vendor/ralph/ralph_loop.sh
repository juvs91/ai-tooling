...
build_claude_command() {
    ...
    # Add Ralph-specific system prompt from .ralphrc RALPH_SYSTEM_PROMPT
    if [[ -n "$RALPH_SYSTEM_PROMPT" && -f "$RALPH_SYSTEM_PROMPT" ]]; then
        local ralph_system_content
        ralph_system_content=$(cat "$RALPH_SYSTEM_PROMPT")
        CLAUDE_CMD_ARGS+=("--append-system-prompt" "$ralph_system_content")
        log_status "INFO" "Loaded Ralph system prompt from: $RALPH_SYSTEM_PROMPT"
    fi
