...
* **Repository:** `ai-tooling` (remotes: `personal` → `github.com/juvs91/ai-tooling`, `deacero` → `bitbucket.org/deacero/ai-tools`)
...
| monorepo, trunk-based, sparse checkout, worktree, tag por proyecto, independencia de despliegue, release por proyecto, ramas compartidas contaminan ambientes, hotfix paralelo | **gitops-monorepo** — Estrategia GitOps company-específica: tag=versión en prod, sparse checkout, worktrees (ADR-0008), hotfix protocol, exportable a otros proyectos. | `infrastructure/gitops-monorepo/SKILL.md` | — |
